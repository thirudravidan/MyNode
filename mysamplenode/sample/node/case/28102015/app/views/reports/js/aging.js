...
apps
.controller('agingcaseController', ['$scope', '$rootScope', '$builder', '$validator', 'AjaxMethod', 'ngTableParams', '$routeParams', '$filter', function($scope, $rootScope, $builder, $validator, AjaxMethod, ngTableParams, $routeParams, $filter) {
    ...
    $scope.dataInit = function (page,type) { 
        ...
        if(type === 'c') {
            ...
            $scope.srch.searchtxt = '';
            AjaxMethod.postMethod('aging/'+page).then(function(response){
                $scope.data = [];
                var dataL = {};
                data = response;
                console.dir(data);
    ...
    $scope.getagingListValue = function(value, key){  
        // if(value) { 
        ...
        if(key === 'dateDifference')
        {
            var days = Math.round(parseInt(value) / 86400000);
            var retResult = (days > 0)? days+' days' : Math.round(parseInt(value)/1440000)+' hours';
            return retResult;
        }
        else
        { 
