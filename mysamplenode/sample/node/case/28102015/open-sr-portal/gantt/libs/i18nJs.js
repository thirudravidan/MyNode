  ...
  Date.defaultFormat = "dd-MMM-yyyy";
...
function isHoliday(date) {
      var ptweekoff = parentgconfig.wekkoff;
      var rawholiday = parentgconfig.holidays;
      var rawSpecialWeekDays = parentgconfig.specialWorking;      
      ...
      var tueIsHoly = false;
      var wedIsHoly = false;
      var thusIsHoly = false;

      $.each(ptweekoff, function (idx, rec) {
          switch (rec) {
              case 0:
                  sunIsHoly = true;
                  break;
      ...
      //  var holidays = "#2015_01_01#2015_01_14#2015_05_01#2015_07_29#2015_08_15#2015_08_29#2015_02_10#2015_10_22#2015_12_25#";
      var holidays = "#";
      if (rawholiday) {
          $.each(rawholiday, function (idx, rec) {
              var date = new Date(Date.parse(rec.holiday_date));
              holidays += date.getFullYear() + "_" + ((date.getMonth() + 1) < 10 ? "0" : "") + (date.getMonth() + 1) + "_" + (date.getDate() < 10 ? "0" : "") + date.getDate() + "#";
          });
      }

      var specialworkingdays = "#";

      if (rawSpecialWeekDays) {
          
          $.each(rawSpecialWeekDays, function (idx, rec) {
              var date = new Date(Date.parse(rec.special_date));
              specialworkingdays += date.getFullYear() + "_" + ((date.getMonth() + 1) < 10 ? "0" : "") + (date.getMonth() + 1) + "_" + (date.getDate() < 10 ? "0" : "") + date.getDate() + "#";
          });
      }
      ...
      var day = date.getDay();
      if (specialworkingdays.indexOf(ymd) > -1 || specialworkingdays.indexOf(md) > -1)
          return false;

      return (day == 1 && monIsHoly) || (day == 2 && tueIsHoly) || (day == 3 && wedIsHoly) || (day == 4 && thusIsHoly) || (day == 5 && friIsHoly) || (day == 6 && satIsHoly) || (day == 0 && sunIsHoly) || holidays.indexOf(ymd) > -1 || holidays.indexOf(md) > -1;
  }
